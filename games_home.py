...
def opp_pitch_ab(opp_pitcher, game):
    # Set away pitcher index and divide sertain stats by number PA
    opp_pitcher = opp_pitcher.set_index(pd.Index([game[4]]))
...
def stat_per_ab(bat_vs_pitcher_hand, game):
    ...
    bat_vs_pitcher_hand = pd.DataFrame([bat_vs_pitcher_hand], index = pd.Index([game[1]]))
...
for game in scores_weather.itertuples():
    if game[19] in list(pitchers['NAME']) and game[19] in list(pitchers_vs_batterside['NAME']):
        team_bat = pd.DataFrame([team_offense.loc[game[1]]],  index = pd.Index([game[1]]), columns = team_offense.columns)  # Home Team
        team_def = pd.DataFrame([defensive_efficiency.loc[game[4]]], index = pd.Index([game[4]]), columns = defensive_efficiency.columns)  # Away Team        
        opp_pitcher = pitchers[pitchers['TEAM'] == game[4]][pitchers['NAME'] == game[19]]  # Away Team
        bat_vs_pitcher_hand = pitcher_hand(game)[pitcher_hand(game)['TEAM'] ==  game[1]]  # Home Team
        park_factor = park_factor_hand[park_factor_hand['TEAM'] == game[4]]  # Home Stadium
        bullpen1 = pd.DataFrame([bullpen.loc[game[4]].values], index = pd.Index([game[4]]), columns = bullpen.columns)  # Away Team        
        ...
        bat_team = batters[batters['TEAM'] == game[1]].sort_values(by = ['PA'], ascending = False)  # Home Team
        ...
        home_merge = home_merge.set_index(pd.Index([game[4]]))
        home_away = away_merge.join(home_merge)
        game_ind = (game[22] + ' ' + home_away.index[0] + ' ' + game[1])
        home_away = home_away.set_index(pd.Index([game_ind]))
        weather = pd.DataFrame([[game[2], game[3], game[7], game[9], game[10], game[11], game[12], game[13]]], columns = ['Proj_Runs', 'Runs', 'Time', 'Temp', 'Humidity', 'Wind', 'Direction', 'Location'])
...
games2.to_csv('CSV/game_home3.csv', mode='w', index=True, header=True)
